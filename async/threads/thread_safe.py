...
def do_transfer_global_lock(from_account: Account, to_account: Account, amount: int):
...
def do_transfer(from_account: Account, to_account: Account, amount: int):
    ...
    """
    fine-grain implementation to Rlock() is fine for this do_transfer(),
    we require transfers to always go from acount A --> acount B, as allowing
    A --> B, and B --> A results in the locks criss-crossing. 

    Speed performance is not hindered too much here. 
    """
    lock1, lock2 = (
...
def validate_bank(accounts: List[Account], total: int, quiet=False):
    ...
    """
    Fine-grain implementation of Rlock - each acount has a lock we need to check,
    This hinders speed performance and grows linearly for the amount of acounts being
    processed. 
    """
